...
public class Prog_Ev02_Ej09 {
    ...
    public static void main(String[] args) {
        ...
        datos = isDoping(datos);
        datos = menor(datos);
        datos = pagar(datos);

        for (int i = 0; i < 3; i++) {
            System.out.println("En el puesto nº " + (i + 1) + ", el dorsal: " + datos[i][0]);
        }
    }
    ...
    static int[][] isDoping(int[][] dopaje) {
        int cont = 0;
        for (int i = 0; i < dopaje.length; i++) {
            if (dopaje[i][2] == 0) {
                cont++;
                dopaje[i][0] = -1;
            }
        ...
        int[][] antid = new int[dopaje.length - cont][4];

        int cont2 = 0;
        for (int i = 0; i < dopaje.length; i++) {
            if (dopaje[i][0] != -1) {
                for (int j = 0; j < dopaje[i].length; j++) {

                    antid[i - cont2][j] = dopaje[i][j];
                }
            } else {
                cont2++;
            }
